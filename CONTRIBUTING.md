...
- **Cite a source** for any correction or addition, ideally a link to the official Git documentation (git-scm.com/docs), the relevant `man` page, or `git help <command>` output.
- Prefer the official docs over blog posts or AI output.
- If behavior is version-dependent, say so (which Git version).

## How it works

- The whole cheatsheet is one static page: **`public/index.html`**. To add or fix a command, edit the relevant table row (`<tr>`) or card there.
